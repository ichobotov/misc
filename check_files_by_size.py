...
with open ('result.txt', 'w') as f:
    f.write(f'Failed comnav files: {comnav_fail} ---> {round((comnav_fail / comnav_files) * 100, 2)}%'+'\n\r')
    f.write(f'Failed radio files: {radio_fail} ---> {round((radio_fail / radio_files) * 100, 2)}%')
